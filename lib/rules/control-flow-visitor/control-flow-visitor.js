...
export const controlFlowVisitor = (config) => createRule({
    defaultSeverity: "error",
    defaultValue: "__DEFAULT_MAIN_VALUE__",
    verify: verifyFactory(config),
});
const verifyFactory = (config) => async (context) => await verify(context, config);
const verify = async (context, config) => {
